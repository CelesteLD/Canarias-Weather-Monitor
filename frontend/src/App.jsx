  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--bg)', overflow: 'hidden' }}>
      ...
      <header style={{
        ...
        background: 'rgba(12,20,36,0.97)', backdropFilter: 'blur(12px)',
        zIndex: 100, flexWrap: 'wrap', gap: 10,
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ width: 34, height: 34, borderRadius: 9, background: 'linear-gradient(135deg, #0ea5e9, #38bdf8)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 17, flexShrink: 0 }}>🌊</div>
          <div>
            <div style={{ fontFamily: 'var(--font-head)', fontWeight: 800, fontSize: 16, letterSpacing: '-0.02em' }}>Canarias Weather Monitor</div>
            <div style={{ color: 'var(--muted)', fontSize: 9, letterSpacing: '0.06em', textTransform: 'uppercase' }}>Spark Structured Streaming · Tiempo Real</div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
          ...
          {/* Tabs de modo */}
          <div style={{ display: 'flex', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 9, padding: 3, gap: 3 }}>
            {TABS.map(({ key, icon, label }) => {
              const active = mode === key
              const colors = { weather: '#0369a1', air: '#a21caf', temp: '#065f46' }
              const textColors = { weather: '#38bdf8', air: '#e879f9', temp: '#34d399' }
              return (
                <button key={key} onClick={() => setMode(key)} style={{
                  padding: '5px 12px', borderRadius: 7, cursor: 'pointer',
                  fontFamily: 'var(--font-mono)', fontSize: 11, fontWeight: 600,
                  background: active ? colors[key] + '55' : 'transparent',
                  color: active ? textColors[key] : 'var(--muted)',
                  border: active ? `1px solid ${colors[key]}` : '1px solid transparent',
          ...
          {!isTempMode && mode === 'weather' && [
            ...
            <div key={level} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '4px 12px', borderRadius: 8, background: bg, border: `1px solid ${color}22` }}>
              <span style={{ color, fontFamily: 'var(--font-mono)', fontWeight: 700, fontSize: 18 }}>{weatherCounts[level]}</span>
      ...
      {!isTempMode && (
        <div style={{ flex: 1, display: 'flex', overflow: 'hidden', minHeight: 0 }}>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', borderRight: '1px solid var(--border)' }}>
            <div style={{ padding: '7px 14px 2px', color: 'var(--muted)', fontSize: 9, letterSpacing: '0.12em', textTransform: 'uppercase', flexShrink: 0 }}>
            ...
            <div style={{ textAlign: 'center', color: 'var(--muted)', fontSize: 9, padding: '2px 0', flexShrink: 0 }}>
              Clic en una isla · Scroll para zoom
            </div>

            {/* Barra resumen */}
            <div style={{ flexShrink: 0, borderTop: '1px solid var(--border)', padding: '10px 16px', display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 6 }}>
              {weather.map(w => {
                ...
                return (
                  <div key={w.island} onClick={() => setSelectedIsland(w.island)} style={{
                    ...
                    border: `1px solid ${isSel ? colors[lvl] : 'var(--border)'}`,
                    textAlign: 'center', transition: 'all 0.2s',
                  }}>
                    <div style={{ color: colors[lvl], fontFamily: 'var(--font-mono)', fontWeight: 700, fontSize: 14 }}>{val}{unit}</div>
                    <div style={{ color: 'var(--muted)', fontSize: 9, marginTop: 1, fontFamily: 'var(--font-head)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          ...
          <div style={{ width: 'clamp(280px, 25vw, 400px)', display: 'flex', flexDirection: 'column', overflow: 'hidden', flexShrink: 0 }}>
            <div style={{ flex: 1, overflowY: 'auto', borderBottom: '1px solid var(--border)', background: 'var(--surface2)' }}>
            ...
            <div style={{ flex: '0 0 auto', maxHeight: '35%', overflowY: 'auto', background: 'var(--surface2)' }}>
              <div style={{ padding: '9px 14px', borderBottom: '1px solid var(--border)', fontFamily: 'var(--font-head)', fontWeight: 700, fontSize: 10, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--muted)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0, background: 'var(--surface2)', zIndex: 1 }}>
