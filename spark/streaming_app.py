...
from pyspark.sql.functions import from_json, col, window, avg, max as spark_max, min as spark_min, to_timestamp
...
VISIBILITY_LOW = 1000
PRESSURE_DROP  = 3.0
STORM_MIN, STORM_MAX = 200, 299

# Umbrales calidad del aire (µg/m³)
...
def compute_weather_alert(r):
    ...
    warning = (
        ...
        (r.visibility is not None and r.visibility < VISIBILITY_LOW)
    )
...
def build_weather_alerts(r, pressure_drop):
    ...
    if r.weather_id and STORM_MIN <= r.weather_id <= STORM_MAX: alerts.append(f"⛈️ Tormenta: {r.weather_desc}")
    return alerts
...
def send_to_backend(batch_df, batch_id):
    ...
    for item in payload:
        ...
        print(f"[SPARK] {item['island']:25s} | {item['temp']:5.1f}°C | "
              f"AQI:{item['aqi']} {item['aqi_label'] or '—':10s} | "
              f"PM2.5:{item['pm2_5'] or '—'} | Meteo:{lvl_w} Aire:{lvl_a}")
...
def main():
    ...
    parsed = raw.select(from_json(col("value"), SCHEMA).alias("d")).select("d.*") \
    ...
    windowed = parsed \
        ...
        .groupBy(
            ...
            col("temp"), col("feels_like"), col("humidity"), col("pressure"),
            col("wind_speed"), col("wind_deg"), col("clouds"), col("visibility"),
